...
const getPreviewText = (jsonString: string, limit: number = 100) => {
  try {
    ...
    if (data && Array.isArray(data.blocks)) {
      ...
      const allText = data.blocks.map((block: any) => block.data?.text || '').join(' ');
...
export const PostList: React.FC<PostListProps> = ({ onPostClick }) => {
  ...
  if (loading)
    return <div style={{ padding: 20, textAlign: 'center', color: '#888' }}>Loading Feed...</div>;
    ...
    >
      ...
      <div style={{ display: 'grid', gap: '20px', marginTop: '20px' }}>
        {filteredPosts.length === 0 && (
          <div style={{ textAlign: 'center', color: '#666', padding: '20px' }}>No posts found.</div>
        )}
        ...
        {filteredPosts.map((post, i) => (
          <motion.div
            ...
            onClick={() =>
              onPostClick({ id: `post_${post.id}`, label: post.title, group: 3, details: post })
            }
            style={{
              background: '#1a1a1a',
              borderRadius: '12px',
          ...
          >
            ...
            <div style={{ padding: '20px' }}>
              ...
              <h2 style={{ margin: '0 0 10px 0', fontSize: '1.5rem', color: 'white' }}>
                {post.title}
              </h2>
