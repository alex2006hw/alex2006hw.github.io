...
export const BlogViewer: React.FC<{ node: any; onClose: () => void }> = ({ node, onClose }) => {
  ...
  useEffect(() => {
    localStorage.setItem('draft_author', author);
  }, [author]);
  useEffect(() => {
  ...
  const contentBlocks = useMemo(() => {
    try {
      ...
      const raw =
        typeof details.content === 'string' ? JSON.parse(details.content) : details.content;
      ...
      >
        ...
        >
          <div style={{ marginBottom: 20 }}>
            <h1 style={{ margin: '0 0 10px 0', color: '#fff', fontSize: '2rem' }}>
              {details.title}
            </h1>
            <div style={{ display: 'flex', justifyContent: 'space-between', color: '#666' }}>
              <span>{details.date}</span>
              <span style={{ color: '#00aaff' }}>{details.tags}</span>
            </div>
          </div>
          ...
          <div
            style={{ lineHeight: '1.8', color: '#ddd', fontSize: '1.1rem', marginBottom: '40px' }}
          >
            {contentBlocks}
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {existingComments.map((c: any, i: number) => (
                ...
                >
                  ...
                  >
                    <strong style={{ color: '#00ff88' }}>{c.author}</strong>
                    <span style={{ fontSize: '0.8rem', color: '#666' }}># {i + 1}</span>
                  </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <button
                ...
                style={{
                  padding: '12px',
                  background: comment.trim() ? '#0070f3' : '#333',
                  border: 'none',
                  color: 'white',
                  fontWeight: 'bold',
                  borderRadius: '4px',
