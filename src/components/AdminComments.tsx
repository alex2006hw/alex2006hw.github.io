      ...
      {comments.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {comments.map((c) => (
            <div
              key={c.id}
              style={{ background: '#222', padding: 15, borderRadius: 5, border: '1px solid #444' }}
            >
              <div style={{ marginBottom: 5 }}>
                <strong>{c.author}</strong> on Post #{c.post_id}
              </div>
              <div style={{ color: '#ccc', fontStyle: 'italic', marginBottom: 10 }}>
                "{c.content}"
              </div>
