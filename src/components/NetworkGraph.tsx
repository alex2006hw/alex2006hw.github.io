      ...
      {!isLoading && graphData.nodes.length > 0 && (
        <ForceGraph3D
          ...
          nodeThreeObject={(node: any) => {
            if (node.group === 'root')
              return new THREE.Mesh(new THREE.SphereGeometry(10), materials.root);
        ...
        {selectedTags.size > 0 && (
          ...
          >
            ...
            {matchedPosts.length === 0 && (
              <div style={{ color: '#666', fontSize: '0.8rem' }}>Try selecting different tags.</div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {matchedPosts.map((post) => (
                <div
                  key={post.id}
                  onClick={() =>
                    onNodeClick({ id: post.id, label: post.label, details: post.details, group: 3 })
                  }
                  style={{
                    padding: '10px',
                    background: '#111',
